...
TEST(matrix_mirroring, test1) {
	...
	EXPECT_EQ(matrix_mirroring(nullptr, 1, 1, matrix_destination), IZ2_ERROR_MEMORY);
	EXPECT_EQ(matrix_mirroring(matrix_source, 1, 1, nullptr), IZ2_ERROR_MEMORY);
	EXPECT_EQ(matrix_mirroring(matrix_source, 0, 1, matrix_destination), IZ2_ERROR_ARGUMENT);
	EXPECT_EQ(matrix_mirroring(matrix_source, 1, 0, matrix_destination), IZ2_ERROR_ARGUMENT);
	EXPECT_EQ(matrix_mirroring(matrix_source, 1, 1, matrix_destination), IZ2_OK);
	EXPECT_EQ(matrix_destination[0], 123);
...
TEST(matrix_mirroring, test2) {
	...
	EXPECT_EQ(matrix_mirroring(matrix_source, rows_count, columns_count, matrix_destination), IZ2_OK);
	const int *matrix_destination_correct = new int[rows_count * columns_count]{8, 5, 2, 7, 4, 1, 6, 3, 0};
	EXPECT_TRUE(std::equal(matrix_destination_correct, matrix_destination_correct + rows_count * columns_count,
	                       matrix_destination));
	delete[] matrix_source;
	delete[] matrix_destination;
